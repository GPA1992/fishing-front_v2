...
export default function PlanningData() {
  ...
  return (
    <section className="relative z-20 h-full w-full overflow-hidden rounded-2xl border border-[color-mix(in_srgb,var(--color-border)_75%,var(--color-primary)_25%)] bg-surface shadow-[0_18px_38px_rgba(0,0,0,0.06)]">
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="relative flex flex-wrap items-center gap-2">
                  {targetDate && (
                    <span className="inline-flex items-center gap-2 rounded-full bg-[color-mix(in_srgb,var(--color-surface-muted)_90%,white_10%)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-primaryStrong ring-1 ring-[color-mix(in_srgb,var(--color-border)_70%,var(--color-primary)_15%)] shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
                      <span className="h-1.5 w-1.5 rounded-full bg-(--color-primary-strong)" />
                      Data · {dateLabel}{" "}
